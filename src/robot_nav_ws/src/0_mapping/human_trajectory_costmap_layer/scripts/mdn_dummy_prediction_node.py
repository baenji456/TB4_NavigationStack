...
class MdnDummyPredictionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        raw_conf = self.get_parameter('confidence_levels').value
        try:
            self.confidence_levels: List[float] = [float(x) for x in raw_conf]
        except Exception:
            self.get_logger().warn(
                "Parameter 'confidence_levels' ist kein Array – fallback auf [0.68, 0.95]."
            )
        ...
        self.publish_enabled: bool = bool(self.get_parameter('publish_enabled').value)
        self.visualize_enabled: bool = bool(self.get_parameter('visualize_enabled').value)
